  ...
  React.useEffect(() => {
    ...
    const loadDashboard = async () => {
      try {
        ...
        const [ridesResponse, usersResponse, savingsResponse] = await Promise.all([
          getRidesDashboard(),
          getTotalUsers(),
          meId ? getUserSavings(meId) : Promise.resolve({data: null}),
        ]);
        ...
        setSavings(
          savingsResponse?.data?.savings !== undefined ? Number(savingsResponse.data.savings) : null
        );
      } catch {
        if (!isMounted) return;
        setRides([]);
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 w-full max-w-6xl">
              <Card className="hover:shadow-md transition-shadow">
                ...
                <CardContent className="text-center">
                  <div className="text-3xl font-bold">{activeRidesCount}</div>
              ...
              <Card className="hover:shadow-md transition-shadow">
                ...
                <CardContent className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Your total savings</p>
                </CardContent>
              </Card>
              <ActiveNowCard />
            </div>
          ...
          <div className="w-full max-w-5xl mx-auto bg-muted/50 min-h-screen flex-1 rounded-xl md:min-h-min p-3">
            <div className="flex flex-col items-center">
              {rides.map((ride) => {
                ...
                const priceIfYouJoin = pricePerRide / (occupiedSeats + 1);
                return (
                      ...
                      <CardTitle className="flex w-full justify-between text-sm font-medium">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-10 w-10 rounded-full">
                            <AvatarImage src="/avatars/shadcn.jpg" alt={ride.driver.name} />
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-1 text-sm text-muted-foreground">
                              ...
                              <span>{averageRating(ride.driver.ratingsGot)}</span>
                            </div>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-muted-foreground">Ride total: €{pricePerRide.toFixed(2)}</p>
                          <div className="text-2xl font-bold text-primary">€{priceIfYouJoin.toFixed(2)}</div>
                          <p className="text-xs text-muted-foreground flex items-center justify-end gap-1">
                            <Users className="h-3 w-3" /> {occupiedSeats + 1} splitting
                          </p>
                        </div>
                      </CardTitle>
                    </CardHeader>
