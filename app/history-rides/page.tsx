...
export default function HistoryRidesPage() {
    ...
    const [viewMode, setViewMode] = useState<"passenger" | "driver">(
        "passenger"
    );
    ...
    const [confirmDeleteRideId, setConfirmDeleteRideId] = useState<
        number | null
    >(null);
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                if (!meId) return;
                const normalizedMeId =
                    typeof meId === "string" ? Number(meId) : meId;
                setCurrentUserId(
                    Number.isFinite(normalizedMeId) ? normalizedMeId : null
                );
    ...
    const handleCancelBooking = async (rideId: number, passengerId: number) => {
        ...
        try {
            ...
            setBookings((prev) =>
                prev.filter(
                    (b) =>
                        !(b.ride.id === rideId && b.passengerId === passengerId)
                )
            );
        } catch (error) {
    ...
    const handleDeleteRide = async (rideId: number) => {
        ...
        try {
            ...
            setPostedRides((prev) =>
                prev.map((ride) =>
                    ride.id === rideId ? { ...ride, status: "Canceled" } : ride
                )
            );
        } catch (error) {
            console.error("Failed to cancel ride", error);
            showAlert("Danger", "Can't cancel the ride.");
        } finally {
            setWorkingRide(null);
        }
    };
    ...
    const getBookingToken = (rideId: number, passengerId: number) =>
        `${rideId}-${passengerId}`;
    ...
    const renderRideCard = (item: BookingWithRide) => {
        const isSelfDriver =
            currentUserId !== null && item.ride.driverId === currentUserId;
        ...
        const existingRating = item.ride.ratings?.find(
            (rating) =>
                String(rating.userPassengerId) === String(currentUserId ?? "")
        );
        const ratingValue = existingRating?.rating ?? 0;
        const canRate = isCompleted && !existingRating;
        const isHovering =
            hoveredRideId === item.ride.id && hoveredRating !== null;
                ...
                <CardContent className="space-y-4 pt-6">
                    <div className="flex items-center justify-between">
                        ...
                        <p className="text-sm text-muted-foreground">
                            {formatDate(item.ride.dateTime)},{" "}
                            {formatTime(item.ride.dateTime)}
                        </p>
                    </div>
            ...
            <SidebarInset className="p-6">
                {confirmDeleteRideId !== null ? (
                    ...
                    >
                        ...
                        >
                            <h3
                                id="confirm-cancel-ride-title"
                                className="text-lg font-semibold"
                            >
                                Cancel ride
                            </h3>
                            <p
                                id="confirm-cancel-ride-description"
                            ...
                            <div className="mt-5 flex justify-end gap-2">
                                <Button
                                    ...
                                    disabled={
                                        workingRide === confirmDeleteRideId
                                    }
                                >
                                    {workingRide === confirmDeleteRideId
                                        ? "Deleting..."
